                  ...
                  <Table>
                    <TableHeader>
                      <TableRow>
                        ...
                        {isAdmin && (
                          <TableHead>{t('Platform Retained')}</TableHead>
                        )}
                    ...
                    <TableBody>
                      {(stats?.details ?? []).map((row) => (
                        <TableRow key={`${row.customer_user_id}-${row.group}`}>
                          ...
                          <TableCell>
                            {formatQuota(row.consumption_quota)}
                          </TableCell>
                          <TableCell>
                          ...
                          <TableCell>
                            {formatQuota(row.gross_profit_quota)}
                          </TableCell>
                          {isAdmin && (
                            <TableCell>
                              {formatQuota(row.platform_retained_quota ?? 0)}
                            </TableCell>
                          )}
                          <TableCell className='font-medium'>
                            {formatQuota(row.agent_earnings_quota)}
                          </TableCell>
                        </TableRow>
                      ))}
                  ...
                  <div className='flex items-center justify-between border-t p-3'>
                    ...
                    <div className='flex gap-2'>
                      ...
                      <Button
                        variant='outline'
                        size='sm'
                        disabled={page * 20 >= (stats?.total ?? 0)}
                        onClick={() => setPage((value) => value + 1)}
                      >
                        {t('Next')}
                      </Button>
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {settlements.map((row) => (
                        <TableRow key={row.id}>
                          <TableCell>
                            {formatTimestamp(row.period_start)} —{' '}
                            {formatTimestamp(row.period_end)}
                          </TableCell>
                          <TableCell className='font-medium'>
                            {formatQuota(row.agent_earnings_quota)}
                          </TableCell>
        ...
        user={
          activeProfile
            ? ({
                ...
                request_count: 0,
                group: '',
                agent_enabled: activeProfile.enabled,
              } as User)
            : undefined
        }
        onSaved={() => {
        ...
        <DialogContent>
          ...
          <div className='space-y-4'>
            <div className='space-y-1.5'>
              ...
              <DateTimePicker
                value={cutoff}
                onChange={(date) => date && setCutoff(date)}
              />
            </div>
            <div className='space-y-1.5'>
              <Label htmlFor='agent-payment-reference'>
                {t('Payment Reference')}
              </Label>
              <Textarea
                id='agent-payment-reference'
