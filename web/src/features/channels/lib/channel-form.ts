...
export const MAX_CHANNEL_RETRY_TIMES = 3
...
export const channelFormSchema = z
  .object({
    ...
    force_image_b64_json_no_url: z.boolean().optional(), // OpenAI image APIs
    image_response_url_prefix: z.string().optional(), // OpenAI image APIs
    normalize_openai_image_response: z.boolean().optional(), // OpenAI image APIs
    replace_video_urls_with_proxy: z.boolean().optional(), // OpenAI/Sora video APIs
...
export const CHANNEL_FORM_DEFAULT_VALUES: ChannelFormValues = {
  ...
  normalize_openai_image_response: false,
  replace_video_urls_with_proxy: false,
...
export function transformChannelToFormDefaults(
  ...
  if (channel.setting) {
    try {
      ...
      extraSettings = {
        ...
        retry_times:
          typeof parsed.retry_times === 'number' ? parsed.retry_times : 0,
        proxy: parsed.proxy || '',
  ...
  let forceImageB64JSONNoURL = false
  let imageResponseURLPrefix = ''
  let normalizeOpenAIImageResponse = false
  let replaceVideoURLsWithProxy = false
  ...
  if (channel.settings) {
    try {
      ...
      forceImageB64JSONNoURL = parsed.force_image_b64_json_no_url === true
      imageResponseURLPrefix =
        typeof parsed.image_response_url_prefix === 'string'
          ? parsed.image_response_url_prefix
          : ''
      normalizeOpenAIImageResponse =
        parsed.normalize_openai_image_response === true
      replaceVideoURLsWithProxy = parsed.replace_video_urls_with_proxy === true
  ...
  return {
    ...
    force_image_b64_json_no_url: forceImageB64JSONNoURL,
    image_response_url_prefix: imageResponseURLPrefix,
    normalize_openai_image_response: normalizeOpenAIImageResponse,
    replace_video_urls_with_proxy: replaceVideoURLsWithProxy,
...
function buildSettingsJSON(formData: ChannelFormValues): string {
  ...
  if (formData.type === 1 || formData.type === 57) {
    ...
    settingsObj.force_image_b64_json_no_url =
      formData.force_image_b64_json_no_url === true
    settingsObj.image_response_url_prefix =
      formData.image_response_url_prefix?.trim() || ''
    ...
    if ('force_image_b64_json_no_url' in settingsObj) {
      delete settingsObj.force_image_b64_json_no_url
    }
    if ('image_response_url_prefix' in settingsObj) {
      delete settingsObj.image_response_url_prefix
    }
  ...
  if (OPENAI_IMAGE_RESPONSE_CHANNEL_TYPES.has(formData.type)) {
    settingsObj.normalize_openai_image_response =
      formData.normalize_openai_image_response === true
  } else if ('normalize_openai_image_response' in settingsObj) {
    delete settingsObj.normalize_openai_image_response
  }

  if (formData.type === 1 || formData.type === 55) {
    settingsObj.replace_video_urls_with_proxy =
      formData.replace_video_urls_with_proxy === true
  } else if ('replace_video_urls_with_proxy' in settingsObj) {
    delete settingsObj.replace_video_urls_with_proxy
  }
