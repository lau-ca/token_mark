...
export function getRedemptionFormSchema(t: TFunction) {
  ...
  return z
    ...
    .superRefine((data, ctx) => {
      ...
      if (!data.subscription_plan_id || data.subscription_plan_id <= 0) {
        ctx.addIssue({
...
export function transformFormDataToPayload(
  ...
  const benefitType = data.benefit_type || 'quota'
  return {
    name: data.name,
    ...
    subscription_plan_id:
      benefitType === 'subscription' ? Number(data.subscription_plan_id || 0) : 0,
