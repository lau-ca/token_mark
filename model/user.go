...
func (options UserSortOptions) Apply(query *gorm.DB) *gorm.DB {
	...
	q := query.Order(clause.OrderByColumn{
		Column: clause.Column{Name: "pinned_at"},
		Desc:   true,
...
func (user *User) UpdateWithTx(tx *gorm.DB, updatePassword bool) error {
	...
	if err = tx.Model(&current).Omit("quota", "used_quota", "request_count", "pinned_at", "auth_version").Updates(newUser).Error; err != nil {
		return err
	}
...
func UpdateUserUsedQuota(id int, quota int) { UpdateUserUsedQuotaAndRequestCount(id, quota) }
